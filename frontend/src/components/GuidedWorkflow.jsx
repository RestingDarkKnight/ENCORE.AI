// GuidedWorkflow.jsx — the inline 6-agent case-design workspace.
//
// Mounted inline below the mode/time/notes card on /roles/:id. Replaces the
// retired one-shot generate flow. Each step is one Claude call; the user
// reviews and clicks Continue / Revise / Give input. Implements: persistent
// progress strip, per-step reasoning panel, honest agent labels, feedback
// routing on every action. Resumes the newest in-progress workflow for the
// role; otherwise starts fresh with the mode/time/notes the user selected
// in the card above.
...
import {
  ArrowRight, ArrowsClockwise, Brain, Atom, Shapes, NotePencil, ShieldCheck,
  Sparkle, CheckCircle, CaretDown, CaretUp, CircleNotch, Lightning, Warning,
...
export default function GuidedWorkflow({
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const list = await api.get(`/workflows/by-role/${role.id}`).catch(() => ({ data: [] }));
        ...
        const validMode = ["screening", "takehome", "interview"].includes(initialMode) ? initialMode : "interview";
        const fresh = await api.post(`/workflows`, {
          role_id: role.id,
          assessment_mode: validMode,
          require_reasoning: !!initialReasoning,
          estimated_minutes: initialMinutes ?? null,
          notes: initialNotes || "",
        });
        if (cancelled) return;
        setWf(fresh.data);
        ...
        if (!cancelled) toast.error(e?.response?.data?.detail || "Failed to start workflow.");
      }
    })();
    return () => { cancelled = true; };
  }, [role?.id, initialMode, initialReasoning, initialMinutes, initialNotes, resumeOnly]);
  ...
  const finalize = useCallback(async () => {
    ...
    try {
      ...
      toast.success("Case saved as a draft.");
      if (onFinalized) onFinalized(data.case_id);
    } catch (e) {
  ...
  if (!wf) {
    return (
      <div className="encore-card p-8 flex items-center gap-3 text-ink-soft" data-testid="guided-loading">
        <CircleNotch size={18} className="animate-spin text-brand" />
        <p>Spinning up the workflow&hellip;</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6" data-testid="guided-workspace">
      <header className="flex items-center justify-between gap-3 flex-wrap">
        <div className="flex items-center gap-2 flex-wrap">
          <p className="encore-overline">Six agents, one at a time &mdash; you approve each step before the next runs.</p>
        </div>
        <div className="flex items-center gap-2 flex-wrap">
          {wf.assessment_mode && (
            <span data-testid="guided-mode-badge" className="inline-flex items-center gap-1 text-[10px] font-bold uppercase tracking-wider border border-brand/30 bg-brand/[0.05] text-brand rounded-full px-2 py-0.5">
              {MODE_LABEL[wf.assessment_mode] || wf.assessment_mode}{wf.estimated_minutes ? ` · ${wf.estimated_minutes} min` : ""}
            </span>
          )}
          {wf.require_reasoning && (
            <span data-testid="guided-reasoning-badge" className="inline-flex items-center gap-1 text-[10px] font-bold uppercase tracking-wider border border-brand-sand/30 bg-brand-sand/[0.08] text-brand-sand rounded-full px-2 py-0.5">
              + Reasoning
            </span>
          )}
        </div>
...
function ActiveStep({ step, busy, output, reasoning, memoryUsedCount, runStep, sendFeedback, finalize }) {
...
function ArchitectOutput({ output, runStep, sendFeedback, busy }) {
  ...
  return (
    <div className="space-y-4">
      <p className="text-sm text-ink-soft">Pick the shape you want &mdash; or click Variant for a fresh set.</p>
...
function StepFooter({ busy, primary, revise }) {
  return (
